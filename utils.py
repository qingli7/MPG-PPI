...
class Metrictor_PPI:
    ...
    def show_result(self, is_print=False, file=None):
        ...
        aupr_entry_1 = self.tru # the true labels 
        aupr_entry_2 = self.true_prob # the predicted probabilities
        aupr = np.zeros(7)
        for i in range(7):
            precision, recall, _ = precision_recall_curve(aupr_entry_1[:,i], aupr_entry_2[:,i])
            aupr[i] = auc(recall,precision)
        self.Aupr = aupr # Area Under the Precision-Recall Curve (AUPR) 
...
def get_bfs_sub_graph(ppi_list, node_num, node_to_edge_index, sub_graph_size):
    ...
    # selecting a random node from the graph until it finds one with 5 or fewer edges
...
def get_dfs_sub_graph(ppi_list, node_num, node_to_edge_index, sub_graph_size):
    ...
    # selected edges equals the desired subgraph size
    while len(selected_edge_index) < sub_graph_size:
        cur_node = stack[-1]
