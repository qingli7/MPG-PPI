...
from gnn_models_multi import ppi_model
...
parser.add_argument('--ppi_path', default='./protein_info/protein.actions.SHS27k.STRING.pro2.txt', type=str,
                    help="ppi path")
parser.add_argument('--pseq_path', default='./protein_info/protein.SHS27k.sequences.dictionary.pro3.tsv', type=str,
                    help="protein sequence path")
parser.add_argument('--vec_path', default='./protein_info/vec5_CTC.txt', type=str,
                    help='protein sequence vector path')
parser.add_argument('--p_feat_matrix', default='./protein_info/x_list.pt', type=str,
                    help="protein feature matrix")
parser.add_argument('--p_adj_matrix', default='./protein_info/edge_list_12.npy', type=str,
                    help="protein adjacency matrix")
parser.add_argument('--split', default='random', type=str,
                    help='split method, random, bfs or dfs')
parser.add_argument('--save_path', default='./result_save', type=str,
                    help="save folder")
...
def multi2big_x(x_ori):
    ...
    # print('len x', len(x_cat), x_cat.shape) # 1553 [818995, 7]
    return x_cat[1:, :], x_num_index  # [818995-1, 7]

# e.g. x_num_index = [3, 2, 4]
# => batch = [1, 1, 1, 2, 2, 3, 3, 3, 3]
...
# num_index是一个张量，其中包含了与edge_ori中每个子序列对应的元素数量
def multi2big_edge(edge_ori, num_index):
    ...
    for i in range(1553):
        ...
        if i == 0:
            ...
            zj = num_index[:i].clone().detach() #torch.tensor(num_index[:i])
            offset = zj.sum()
...
def main():
    ...
    ppi_data.split_dataset(train_valid_index_path='./train_val_split_data/train_val_split_1.json', random_new=True, mode=args.split)
    graph = ppi_data.data  # Data(x=self.x, edge_index=self.edge_index.T, edge_attr_1=self.edge_attr)
    ...
    p_x_all = torch.load(args.p_feat_matrix) # 1553 [[0.35441527 0. 0. ...]...]
    p_edge_all = np.load(args.p_adj_matrix, allow_pickle=True)  # 1553 [(0, 0), (0, 1),...]

    p_x_all, x_num_index = multi2big_x(p_x_all)  # transform to torch.tensor  1553 [818995, 7]
    p_edge_all, edge_num_index = multi2big_edge(p_edge_all, x_num_index) # [2, 9644122] 
    ...
    optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=0.05, momentum=0.9, weight_decay=1e-8) 
    ...
    save_path = os.path.join(save_path, "gnn_{}".format('training_seed_.5'))
    # save_path = os.path.join(save_path, "gnn_{}".format('multi_layer'))

    result_file_path = os.path.join(save_path, "valid_results_.5.txt")
...
'''
CUDA_VISIBLE_DEVICES=5 python model_train.py
'''