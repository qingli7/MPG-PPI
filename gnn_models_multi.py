...
class GIN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, train_edge_id, p=0.5):
        ...
        # x = self.fc1(x)
        x = torch.mul(x1, x2) # multiply x1 and x2
        x = self.fc2(x)
...
class GCN(nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        ...
        x = F.relu(x) 
        x = self.bn2(x)
        y = self.sag2(x, edge_index, batch = batch1)
        x2 = y[0]
        batch2 = y[3]
        edge_index = y[1]  
        ...
        x = F.relu(x) 
        x = self.bn3(x)
        y = self.sag3(x, edge_index, batch = batch2)
        x3 = y[0]
        batch3 = y[3]
        edge_index = y[1]
        ...
        x = F.relu(x) 
        x = self.bn4(x)
        y = self.sag4(x, edge_index, batch = batch3)
        x4 = y[0]
        batch4 = y[3]
        edge_index = y[1]

        # len x: 409891 205322 103047 51908
        # shape: torch.Size([409891, 128]) torch.Size([205322, 128]) torch.Size([103047, 128]) torch.Size([51908, 128])
        # x  shape:  torch.Size([770168, 128]) 
        # batch shape:  torch.Size([770168])
        # x = torch.cat([torch.cat([x1, x2], 0),torch.cat([x3, x4], 0)],dim=0)
        # batch = torch.cat((torch.cat([batch1, batch2],dim=0),torch.cat([batch3, batch4],dim=0)),dim=0)
        
        x = torch.cat([x1, x2, x3, x4], dim=0)
        batch = torch.cat([batch1, batch2, batch3, batch4], dim=0)
